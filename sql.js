...
const orderedMedals = (country, field, sortAscending) => {
  const groupByStatement = field ? 'GROUP BY ' + field : '';
  let sortByPresent;
  let sortByType;
  ...
  const sortByStatement = field && sortByPresent ? 'ORDER BY ' + field + ' ' + sortByType : '';
...
const orderedSports = (country, field, sortAscending) => {
  ...
  return `
    WITH total_medals as (
      SELECT COUNT(*) AS 'num_medals'
      FROM GoldMedal
      WHERE country = '${country}'
    )
  
    SELECT GoldMedal.sport,
      COUNT(*) AS 'count',
      100.0 * COUNT(*) / total_medals.num_medals AS 'percent'
    FROM GoldMedal
